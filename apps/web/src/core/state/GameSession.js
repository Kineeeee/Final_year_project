...
export class GameSession {
    ...
    start() {
        ...
        // Single source of truth for entities; created once per session
        ...
        // Effect Events
        this._bind(this.scene.events, 'game:itemActivated', (data) => {
            this._applyItemEffect(data, true);
        });
        this._bind(this.scene.events, 'game:itemDeactivated', (data) => {
            this._applyItemEffect(data, false);
        });
    ...
    _applyItemEffect(data, isActive) {
        let snake;
        if (this.scene.player && this.scene.player.playerId === data.playerId) {
            snake = this.scene.player;
            // Also emit legacy event for UI if needed (Game.js listeners?)
            // The existing UI listener in Game.js or UIScene might depend on 'itemActivated'
            // We should check if we broke that. The original NetworkManager emitted 'itemActivated' ONLY for local player
            // effectively.
            if (isActive) this.scene.events.emit('itemActivated', data);
            else this.scene.events.emit('itemDeactivated', data);
        } else if (this.scene.otherSnakes && this.scene.otherSnakes.has(data.playerId)) {
            snake = this.scene.otherSnakes.get(data.playerId);
        }

        if (snake) {
            effectManager.applyEffect(snake, data.itemId, isActive, data.buffValue);
        }
    }
}
